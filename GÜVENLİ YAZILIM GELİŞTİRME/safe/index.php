<?php
include '../config/safedb.php';
session_start();

$stmt = $database->query('SELECT * FROM posts ORDER BY id DESC');
$posts = $stmt->fetchAll(PDO::FETCH_ASSOC);

function loginController(){
    if(empty($_SESSION['username']) && empty($_SESSION['_token'])){ 
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SESSION['username']) && $_SESSION['_token'] == $_POST['_token']){
    $postAdd = $database->prepare('INSERT INTO posts 	(username,post) VALUES(?,?)');
	$postAdd->execute([$_SESSION['username'],$_POST['_POST']]);
    header('Location: index.php');
}
if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SESSION['usernmae']) && empty($_SESSION['_token'])){
		...
		<aside class="left-side">
			...
			<div class="profile-btn">
				...
				<div>
					...
					<span class="tag">@<?php echo $_SESSION['username'] ?></span>
					</div>
					<div>
					<svg viewBox="0 0 24 24" aria-hidden="true">
						<g>
				...
				<div class="right-column">
					...
					<div>
						<strong>
							...
							<span class="tag">@<?php echo htmlspecialchars($post['username']) ?></span>
							<span class="dot">.</span>
			...
			<div class="trends">
				...
				<div class="trends-items">
					...
					<div class="trends-item">
						...
						<strong class="hashtag">#MerhabaDünya</strong>
